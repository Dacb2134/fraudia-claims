...
import { fetchStats, fetchSiniestros } from '../../services/dashboardService'
import type { Stats, Siniestro } from '../../services/dashboardService'
...
export default function Dashboard({ onVerDetalle, onLogout }: {
  onVerDetalle: () => void
  onLogout: () => void
}) {
      ...
      <aside className="sidebar">
        ...
        <nav style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          {[
            ...
            { icon: 'assignment', label: 'Gestión de Casos' },
            ...
            <>
                      ...
                      return (
                        <div key={p.proveedor} style={{ marginBottom: '1rem' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4 }}>
                            ...
                            <span style={{ fontWeight: 700 }}>{p.alertas_rojas} alertas rojas / {p.total_alertas} total</span>
                          </div>
                    ...
                    {stats?.por_sucursal.slice(0, 6).map((s) => (
                      <div key={s.sucursal} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 13 }}>
                        ...
                        <div style={{ display: 'flex', gap: '0.5rem' }}>
                          ...
                          {s.rojos > 0 && (
                            <span style={{ color: '#ba1a1a', fontSize: 11 }}>({s.rojos} 🔴)</span>
                          )}
                        </div>
                      </div>
                    ))}
              ...
              <section className="table-section">
                <div className="table-header">
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    {[undefined, 'ROJO', 'AMARILLO', 'VERDE'].map((n) => (
                      <button key={n ?? 'todos'} onClick={() => setFiltro(n)}
                        style={{
                    ...
                    <tbody>
                      {casos.map((caso) => (
                        <tr key={caso.id_siniestro}>
                          <td style={{ fontWeight: 700 }}>{caso.id_siniestro}</td>
                          <td>
                          ...
                          <td>
                            <span className={`badge badge-${caso.nivel_riesgo === 'ROJO' ? 'alto' : 'medio'}`}>
                              <span className="material-symbols-outlined" style={{ fontSize: 14 }}>
                                {caso.nivel_riesgo === 'ROJO' ? 'report' : caso.nivel_riesgo === 'AMARILLO' ? 'schedule' : 'check_circle'}
                              </span>
                ...
                <div style={{ padding: '1rem 1.5rem', borderTop: '1px solid #c4c6d3', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#eff4ff' }}>
                  <span style={{ fontSize: 12, color: '#434652' }}>
                    Mostrando {casos.length} de {stats?.semaforo.rojo.total} casos críticos
                  </span>
                </div>
              </section>
            </>
